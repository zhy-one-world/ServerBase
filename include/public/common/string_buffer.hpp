...
namespace boost
{
	...
	{
		template<class T>
		inline void serialize(hld::simple_binary_iarchive& ar, hld::basic_string_buffer<T>& v, const unsigned int version)
		{
		...
		template<class T>
		inline void serialize(hld::simple_binary_oarchive& ar, hld::basic_string_buffer<T>& v, const unsigned int version)
		{
			ar& v.length();
