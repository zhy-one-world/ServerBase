...
static bool unescape_elements (void *data, LDAPURLDesc *ludp)
{
  ...
  if(ludp->lud_filter) {
    ludp->lud_filter = faith_curl_easy_unescape(data, ludp->lud_filter, 0, NULL);
    if(!ludp->lud_filter)
       return FALSE;
  }

  for(i = 0; ludp->lud_attrs && ludp->lud_attrs[i]; i++) {
    ludp->lud_attrs[i] = faith_curl_easy_unescape(data, ludp->lud_attrs[i],
                                            0, NULL);
    if(!ludp->lud_attrs[i])
      return FALSE;
    ludp->lud_attrs_dups++;
  ...
  if(ludp->lud_dn) {
    char *dn = ludp->lud_dn;
    char *new_dn = faith_curl_easy_unescape(data, dn, 0, NULL);
