...
/*
 * This is the internal function faith_curl_easy_cleanup() calls. This should
 * cleanup and free all resources associated with this sessionhandle.
 *
 * NOTE: if we ever add something that attempts to write to a socket or
 * similar here, we must ignore SIGPIPE first. It is currently only done
 * when faith_curl_easy_perform() is invoked.
 */
...
CURLcode Curl_close(struct SessionHandle *data)
{
  ...
  if(m)
    /* This handle is still part of a multi handle, take care of this first
	  ...
	  faith_curl_multi_remove_handle(data->multi, data);

  if(data->multi_easy)
    /* when faith_curl_easy_perform() is used, it creates its own multi handle to
       use and this is the one */
	  faith_curl_multi_cleanup(data->multi_easy);
...
CURLcode Curl_init_userdefined(struct UserDefined *set)
{
  ...
  set->fwrite_func = (faith_curl_write_callback)fwrite;

  /* use fread as default function to read input */
  set->fread_func = (faith_curl_read_callback)fread;
  set->is_fread_set = 0;
  set->is_fwrite_set = 0;
                     ...
                     va_list param)
{
  ...
  case CURLOPT_FILETIME:
     ...
     * later (possibly) become available using faith_curl_easy_getinfo().
     */
  ...
  case CURLOPT_COOKIEFILE:
    ...
    if(argptr) {
      ...
      /* append the cookie file name to the list of file names, and deal with
         them later */
      cl = faith_curl_slist_append(data->change.cookielist, argptr);
      if(!cl) {
        faith_curl_slist_free_all(data->change.cookielist);
  ...
  case CURLOPT_PROGRESSFUNCTION:
     ...
     */
    data->set.fprogress = va_arg(param, faith_curl_progress_callback);
  ...
  case CURLOPT_XFERINFOFUNCTION:
     ...
     */
    data->set.fxferinfo = va_arg(param, faith_curl_xferinfo_callback);
  ...
  case CURLOPT_DEBUGFUNCTION:
     ...
     */
    data->set.fdebug = va_arg(param, faith_curl_debug_callback);
    /*
  ...
  case CURLOPT_HEADERFUNCTION:
     ...
     */
    data->set.fwrite_header = va_arg(param, faith_curl_write_callback);
    break;
  case CURLOPT_WRITEFUNCTION:
    /*
     * Set data write callback
     */
    data->set.fwrite_func = va_arg(param, faith_curl_write_callback);
    if(!data->set.fwrite_func) {
      data->set.is_fwrite_set = 0;
      /* When set to NULL, reset to our internal default function */
      data->set.fwrite_func = (faith_curl_write_callback)fwrite;
    }
    else
  ...
  case CURLOPT_READFUNCTION:
     ...
     * Read data callback
     */
    data->set.fread_func = va_arg(param, faith_curl_read_callback);
    if(!data->set.fread_func) {
      data->set.is_fread_set = 0;
      /* When set to NULL, reset to our internal default function */
      data->set.fread_func = (faith_curl_read_callback)fread;
    }
    else
  ...
  case CURLOPT_SEEKFUNCTION:
     ...
     */
    data->set.seek_func = va_arg(param, faith_curl_seek_callback);
    break;
  ...
  case CURLOPT_CONV_FROM_NETWORK_FUNCTION:
     ...
     */
    data->set.convfromnetwork = va_arg(param, faith_curl_conv_callback);
    break;
  case CURLOPT_CONV_TO_NETWORK_FUNCTION:
    /*
     * "Convert to network encoding" callback
     */
    data->set.convtonetwork = va_arg(param, faith_curl_conv_callback);
    break;
  case CURLOPT_CONV_FROM_UTF8_FUNCTION:
    /*
     * "Convert from UTF-8 encoding" callback
     */
    data->set.convfromutf8 = va_arg(param, faith_curl_conv_callback);
    break;
  case CURLOPT_IOCTLFUNCTION:
    /*
     * I/O control callback. Might be NULL.
     */
    data->set.ioctl_func = va_arg(param, faith_curl_ioctl_callback);
    break;
  ...
  case CURLOPT_SSL_CTX_FUNCTION:
     ...
     */
    data->set.ssl.fsslctx = va_arg(param, faith_curl_ssl_ctx_callback);
    break;
  ...
  case CURLOPT_SHARE:
  {
     ...
     * it will be done by faith_curl_easy_perform */
  }
  break;
  ...
  case CURLOPT_SOCKOPTFUNCTION:
     ...
     */
    data->set.fsockopt = va_arg(param, faith_curl_sockopt_callback);
    break;
  ...
  case CURLOPT_OPENSOCKETFUNCTION:
     ...
     * before connect()
     */
    data->set.fopensocket = va_arg(param, faith_curl_opensocket_callback);
    break;
  ...
  case CURLOPT_CLOSESOCKETFUNCTION:
     ...
     */
    data->set.fclosesocket = va_arg(param, faith_curl_closesocket_callback);
    break;
  ...
  case CURLOPT_SSH_KEYFUNCTION:
    ...
    data->set.ssh_keyfunc = va_arg(param, faith_curl_sshkeycallback);
    break;
  ...
  case CURLOPT_INTERLEAVEFUNCTION:
    ...
    data->set.fwrite_rtp = va_arg(param, faith_curl_write_callback);
    break;
  ...
  case CURLOPT_CHUNK_BGN_FUNCTION:
    data->set.chunk_bgn = va_arg(param, faith_curl_chunk_bgn_callback);
    break;
  case CURLOPT_CHUNK_END_FUNCTION:
    data->set.chunk_end = va_arg(param, faith_curl_chunk_end_callback);
    break;
  case CURLOPT_FNMATCH_FUNCTION:
    data->set.fnmatch = va_arg(param, faith_curl_fnmatch_callback);
    break;
  ...
  if(atsign) {
    ...
    if(!res) {
      ...
      if(proxyuser && strlen(proxyuser) < MAX_CURL_USER_LENGTH)
        conn->proxyuser = faith_curl_easy_unescape(data, proxyuser, 0, NULL);
      else
      ...
      if(!conn->proxyuser)
        ...
        if(proxypasswd && strlen(proxypasswd) < MAX_CURL_PASSWORD_LENGTH)
          conn->proxypasswd = faith_curl_easy_unescape(data, proxypasswd, 0, NULL);
        else
                                ...
                                char **user, char **passwd, char **options)
{
  ...
  if(userp) {
    ...
    /* Decode the user */
    newname = faith_curl_easy_unescape(data, userp, 0, NULL);
    if(!newname) {
      result = CURLE_OUT_OF_MEMORY;
      goto out;
    }
  ...
  if(passwdp) {
    ...
    char *newpasswd = faith_curl_easy_unescape(data, passwdp, 0, NULL);
    if(!newpasswd) {
      result = CURLE_OUT_OF_MEMORY;
      goto out;
    }
  ...
  if(optionsp) {
    ...
    char *newoptions = faith_curl_easy_unescape(data, optionsp, 0, NULL);
    if(!newoptions) {
      result = CURLE_OUT_OF_MEMORY;
      goto out;
    }
  ...
  if(proxy) {
    ...
    if((conn->proxytype == CURLPROXY_HTTP) ||
      ...
      if(!(conn->handler->protocol & CURLPROTO_HTTP) &&
        ...
        conn->handler = &faith_Curl_handler_http;
