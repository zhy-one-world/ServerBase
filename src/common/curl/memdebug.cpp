...
#  pragma warning(pop)
  ...
  mem = (faith_Curl_crealloc)(mem, size);
  if(source)
...
void curl_memlog(const char *format, ...)
{
  ...
  buf = (faith_Curl_cmalloc)(LOGLINE_BUFSIZE);
  if(!buf)
    return;
