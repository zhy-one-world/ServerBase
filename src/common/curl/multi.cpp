                                     ...
                                     int chashsize) /* connection hash */
{
  ...
  multi->closure_handle = (SessionHandle*)faith_curl_easy_init();
...
CURLM *faith_curl_multi_init(void)
{
  return Curl_multi_handle(CURL_SOCKET_HASH_TABLE_SIZE,
                           CURL_CONNECTION_HASH_SIZE);
}

CURLMcode faith_curl_multi_add_handle(CURLM *multi_handle,
                                CURL *easy_handle)
{
  ...
  /* Set the timeout for this handle to expire really soon so that it will
     ...
     when only the faith_curl_multi_socket() API is used. During that flow, only
...
CURLMcode faith_curl_multi_remove_handle(CURLM *multi_handle,
                                   CURL *curl_handle)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  ...
  if(easy) {
    ...
    /* The timer must be shut down before data->multi is set to NULL,
       ...
       faith_curl_easy_cleanup is called. */
    Curl_expire(data, 0);
                         ...
                         int numsocks)
{
  /* If the pipe broke, or if there's no connection left for this easy handle,
     ...
     happen when this is called from faith_curl_multi_remove_handle() =>
...
CURLMcode faith_curl_multi_fdset(CURLM *multi_handle,
...
CURLMcode faith_curl_multi_wait(CURLM *multi_handle,
                          ...
                          int *ret)
{
  ...
  if(nfds) {
    ...
    if(i) {
      unsigned int j;
      /* copy revents results from the poll to the faith_curl_multi_wait poll
  ...
  do {
    ...
    case CURLM_STATE_PERFORM:
      ...
      if(data->result) {
        ...
        if(data->easy_conn->recv_pipe->head)
          Curl_expire((SessionHandle*)data->easy_conn->recv_pipe->head->ptr, 1);
    ...
    case CURLM_STATE_DONE:
      ...
      if(data->easy_conn) {
         ...
         * easy_conn to NULL.  In such a case, faith_curl_multi_remove_handle() can
...
CURLMcode faith_curl_multi_perform(CURLM *multi_handle, int *running_handles)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  struct SessionHandle *data;
   ...
   * with unconditionally by this function and faith_curl_multi_timeout() requires
...
CURLMcode faith_curl_multi_cleanup(CURLM *multi_handle)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  struct SessionHandle *data;
...
CURLMsg *faith_curl_multi_info_read(CURLM *multi_handle, int *msgs_in_queue)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  struct Curl_message *msg;
                              ...
                              int *running_handles)
{
  ...
  if(checkall) {
    ...
    result = faith_curl_multi_perform(multi, running_handles);
...
#undef faith_curl_multi_setopt
CURLMcode faith_curl_multi_setopt(CURLM *multi_handle,
                            CURLMoption option, ...)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
  ...
  case CURLMOPT_SOCKETFUNCTION:
    multi->socket_cb = va_arg(param, faith_curl_socket_callback);
    break;
  ...
  case CURLMOPT_TIMERFUNCTION:
    multi->timer_cb = va_arg(param, faith_curl_multi_timer_callback);
    break;
...
/* we define faith_curl_multi_socket() in the public multi.h header */
#undef faith_curl_multi_socket

CURLMcode faith_curl_multi_socket(CURLM *multi_handle, curl_socket_t s,
                            int *running_handles)
{
...
CURLMcode faith_curl_multi_socket_action(CURLM *multi_handle, curl_socket_t s,
...
CURLMcode faith_curl_multi_socket_all(CURLM *multi_handle, int *running_handles)
...
CURLMcode faith_curl_multi_timeout(CURLM *multi_handle,
                             long *timeout_ms)
{
  struct Curl_multi *multi=(struct Curl_multi *)multi_handle;
 ...
 * this'-time for the transfer, to be extracted by faith_curl_multi_timeout()
 *
...
CURLMcode faith_curl_multi_assign(CURLM *multi_handle,
