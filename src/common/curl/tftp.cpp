...
static CURLcode tftp_send_first(tftp_state_data_t *state, tftp_event_t event)
{
  ...
  case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
    ...
    /* As RFC3617 describes the separator slash is not actually part of the
       ...
       string. */
    filename = faith_curl_easy_unescape(data, &state->conn->data->state.path[1], 0,
                                  NULL);
    if(!filename)
...
static CURLcode tftp_rx(tftp_state_data_t *state, tftp_event_t event)
{
  ...
  case TFTP_EVENT_OACK:
    ...
    setpacketevent(&state->spacket, TFTP_EVENT_ACK);
    setpacketblock(&state->spacket, state->block);
    sbytes = sendto(state->sockfd, (char *)state->spacket.data,
                    4, SEND_4TH_ARG,
                    (struct sockaddr *)&state->remote_addr,
                    state->remote_addrlen);
    if(sbytes < 0) {
  ...
  case TFTP_EVENT_ERROR:
    ...
    setpacketblock(&state->spacket, state->block);
    (void)sendto(state->sockfd, (char *)state->spacket.data,
                 4, SEND_4TH_ARG,
                 (struct sockaddr *)&state->remote_addr,
                 state->remote_addrlen);
...
static CURLcode tftp_tx(tftp_state_data_t *state, tftp_event_t event)
{
  ...
  case TFTP_EVENT_ERROR:
    ...
    setpacketevent(&state->spacket, TFTP_EVENT_ERROR);
    setpacketblock(&state->spacket, state->block);
    (void)sendto(state->sockfd, (char *)state->spacket.data, 4, SEND_4TH_ARG,
                 (struct sockaddr *)&state->remote_addr,
                 state->remote_addrlen);
    /* don't bother with the return code, but if the socket is still up we
     * should be a good TFTP client and let the server know we're done */
...
static CURLcode tftp_connect(struct connectdata *conn, bool *done)
{
  ...
  conn->proto.tftpc = state = (tftp_state_data_t *)calloc(1, sizeof(tftp_state_data_t));
  if(!state)
    return CURLE_OUT_OF_MEMORY;
