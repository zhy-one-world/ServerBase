...
struct ssl_config_data {
  ...
  faith_curl_ssl_ctx_callback fsslctx; /* function to initialize ssl ctx */
...
struct connectdata {
  ...
  faith_curl_closesocket_callback fclosesocket; /* function closing the socket(s) */
 ...
 /* connecttime: when connect() is called on the current IP address. Used to
  ...
  faith_curl_seek_callback seek_func; /* function that seeks the input */
  void *seek_client;            /* pointer to pass to the seek() above */

  /*************** Request - specific items ************/

  /* previously this was in the urldata struct */
  faith_curl_read_callback fread_func; /* function that reads the input */
...
struct PureInfo {
  ...
  /* PureInfo members 'conn_primary_ip', 'conn_primary_port', 'conn_local_ip'
     ...
     order to allow faith_curl_easy_getinfo() to return this information even when
...
struct UrlState {
  ...
  /* when faith_curl_easy_perform() is called, the multi handle is "owned" by
     the easy handle so faith_curl_easy_cleanup() on such an easy handle will
  ...
  bool allow_port; /* Is set.use_port allowed to take effect or not. This
                      is always set TRUE when faith_curl_easy_perform() is called. */
...
struct DynamicStatic {
  ...
  struct curl_slist *cookielist; /* list of cookie files set by
                                    faith_curl_easy_setopt(COOKIEFILE) calls */
...
struct UserDefined {
  ...
  faith_curl_write_callback fwrite_func;   /* function that stores the output */
  faith_curl_write_callback fwrite_header; /* function that stores headers */
  faith_curl_write_callback fwrite_rtp;    /* function that stores interleaved RTP */
  faith_curl_read_callback fread_func;     /* function that reads the input */
  int is_fread_set; /* boolean, has read callback been set to non-NULL? */
  int is_fwrite_set; /* boolean, has write callback been set to non-NULL? */
  faith_curl_progress_callback fprogress; /* OLD and deprecated progress callback  */
  faith_curl_xferinfo_callback fxferinfo; /* progress callback */
  faith_curl_debug_callback fdebug;      /* function that write informational data */
  faith_curl_ioctl_callback ioctl_func;  /* function for I/O control */
  faith_curl_sockopt_callback fsockopt;  /* function for setting socket options */
  void *sockopt_client; /* pointer to pass to the socket options callback */
  faith_curl_opensocket_callback fopensocket; /* function for checking/translating
                                           the address and opening the
                                           socket */
  void* opensocket_client;
  faith_curl_closesocket_callback fclosesocket; /* function for closing the
                                             socket */
  void* closesocket_client;

  void *seek_client;    /* pointer to pass to the seek callback */
  /* the 3 faith_curl_conv_callback functions below are used on non-ASCII hosts */
  /* function to convert from the network encoding: */
  faith_curl_conv_callback convfromnetwork;
  /* function to convert to the network encoding: */
  faith_curl_conv_callback convtonetwork;
  /* function to convert from UTF-8 encoding: */
  faith_curl_conv_callback convfromutf8;
  ...
  faith_curl_sshkeycallback ssh_keyfunc; /* key matching callback */
...
/* Here follows boolean settings that define how to behave during
  ...
  faith_curl_chunk_bgn_callback chunk_bgn; /* called before part of transfer
                                        starts */
  faith_curl_chunk_end_callback chunk_end; /* called after part transferring
                                        stopped */
  faith_curl_fnmatch_callback fnmatch; /* callback to decide which file corresponds
