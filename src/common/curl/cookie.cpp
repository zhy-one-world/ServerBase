...
void Curl_cookie_loadfiles(struct SessionHandle *data)
{
  ...
  if(list) {
    ...
    faith_curl_slist_free_all(data->change.cookielist); /* clean up list */
                                       ...
                                       unless set */
{
  ...
  if(httpheader) {
    ...
    if(co->maxage) {
      ...
      co->expires = faith_curl_getdate(co->expirestr, NULL);
    ...
    if(!badcookie && !co->path && path) {
      ...
      if(!queryp)
        ...
        endslash = (char *)memchr(path, '/', (size_t)(queryp - path));
      if(endslash) {
...
struct curl_slist *Curl_cookie_list(struct SessionHandle *data)
{
  ...
  while(c) {
    ...
    if(!line) {
      faith_curl_slist_free_all(list);
      return NULL;
    }
