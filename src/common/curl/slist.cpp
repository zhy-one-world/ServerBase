...
/*
 * faith_curl_slist_append() appends a string to the linked list. It always returns
 * the address of the first record, so that you can use this function as an
 * initialization function as well as an append function. If you find this
 * bothersome, then simply create a separate _init function and call it
 * appropriately from within the program.
 */
struct curl_slist *faith_curl_slist_append(struct curl_slist *list,
                                     const char *data)
{
...
struct curl_slist *Curl_slist_duplicate(struct curl_slist *inlist)
{
  ...
  while(inlist) {
    tmp = faith_curl_slist_append(outlist, inlist->data);

    if(!tmp) {
      faith_curl_slist_free_all(outlist);
      return NULL;
    }
...
void faith_curl_slist_free_all(struct curl_slist *list)
{
