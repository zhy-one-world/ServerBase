...
static CURLcode check_telnet_options(struct connectdata *conn)
{
	...
	if (conn->bits.user_passwd) {
		...
		beg = faith_curl_slist_append(tn->telnet_vars, option_arg);
		if (!beg) {
			faith_curl_slist_free_all(tn->telnet_vars);
			tn->telnet_vars = NULL;
			return CURLE_OUT_OF_MEMORY;
		}
		tn->telnet_vars = beg;
...
static CURLcode telnet_do(struct connectdata *conn, bool *done)
{
...
#ifdef USE_WINSOCK
	HMODULE wsock2;
	typedef bool (WINAPI *close_event)(WSAEVENT hEvent);
	typedef WSAEVENT (WINAPI *create_event)();
	typedef int(WINAPI *event_select)(SOCKET s, WSAEVENT hEventObject, long lNetworkEvents);
	typedef int(WINAPI *enum_netevents)(
		_In_  SOCKET             s,
		_In_  WSAEVENT           hEventObject,
		_Out_ LPWSANETWORKEVENTS lpNetworkEvents);

	close_event close_event_func;
	create_event create_event_func;
	event_select event_select_func;
	enum_netevents enum_netevents_func;
	...
	*/
	...
	create_event_func = (create_event)GetProcAddress(wsock2, "WSACreateEvent");
	...
	/* And WSACloseEvent */
	close_event_func = (close_event)GetProcAddress(wsock2, "WSACloseEvent");
	...
	/* And WSAEventSelect */
	event_select_func = (event_select)GetProcAddress(wsock2, "WSAEventSelect");
	...
	enum_netevents_func = (enum_netevents)GetProcAddress(wsock2, "WSAEnumNetworkEvents");
	...
	*/
		...
		case WAIT_TIMEOUT:
		{
			for (;;) {
				if (obj_count == 1) {
					...
					code = (CURLcode)conn->fread_func(buf, 1, BUFSIZE - 1, conn->fread_in);
	...
	if (conn->fread_func != (faith_curl_read_callback)fread) {
		poll_cnt = 1;
