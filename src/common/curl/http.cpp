 ...
 */
const struct Curl_handler faith_Curl_handler_http = {
  "HTTP",                               /* scheme */
    ...
    *bytes_written += (long)amount;
    ...
    if(http) {
      if((size_t)amount != size) {
        ...
        conn->fread_func = (faith_curl_read_callback)readmoredata;
...
#define TE_HEADER "TE: gzip\r\n"
  ...
  case HTTPREQ_POST_FORM:
    ...
    conn->fread_func = (faith_curl_read_callback)Curl_FormReader;
      ...
      if(!k->header) {
        ...
        if(-1 != k->size) {
          /* We do this operation even if no_body is true, since this
             data might be retrieved later with faith_curl_easy_getinfo()
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
    ...
    /* check for Content-Type: header lines to get the MIME-type */
      ...
      */
      ...
      k->timeofdoc = faith_curl_getdate(k->p+strlen("Last-Modified:"),
                                  &secs);
