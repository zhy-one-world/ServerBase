...
const struct Curl_handler faith_Curl_handler_smtp = {
  "SMTP",                           /* scheme */
...
static CURLcode smtp_init(struct connectdata *conn)
{
  ...
  data->req.protop = smtp = (SMTP *)calloc(sizeof(struct SMTP), 1);
  if(!smtp)
...
{
  struct SMTP *smtp = (SMTP *)conn->data->req.protop;
...
#ifndef CURL_DISABLE_HTTP
    if(conn->handler == &faith_Curl_handler_smtp)
