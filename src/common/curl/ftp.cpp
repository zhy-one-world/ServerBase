...
const struct Curl_handler faith_Curl_handler_ftp = {
  "FTP",                           /* scheme */
...
#endif
  int fcmd_index = fcmd;
  for(; fcmd_index != DONE; fcmd_index++) {

    if(!conn->bits.ftp_use_eprt && (EPRT == fcmd_index))
      /* if disabled, goto next */
      continue;

    if((PORT == fcmd_index) && sa->sa_family != AF_INET)
      /* PORT is ipv4 only */
      continue;
    ...
    if(EPRT == fcmd_index) {
      ...
      result = Curl_pp_sendf(&ftpc->pp, "%s |%d|%s|%hu|", mode[fcmd_index],
                             sa->sa_family == AF_INET?1:2,
                             myhost, port);
      if(result) {
        failf(data, "Failure sending EPRT command: %s",
			faith_curl_easy_strerror(result));
        Curl_closesocket(conn, portsock);
      ...
      *dest = 0;
      ...
      result = Curl_pp_sendf(&ftpc->pp, "%s %s", mode[fcmd_index], tmp);
      if(result) {
                            ...
                            bool *magicdone)
{
  ...
  *magicdone = FALSE;
  ...
  if(conn->bits.tunnel_proxy && conn->bits.httpproxy) {
    ...
    if(conn->tunnel_state[SECONDARYSOCKET] != 2) {
                                    ...
                                    int ftpcode)
{
  ...
  if(ftpcode != 200) {
    ...
    if(fcmd == DONE) {
      ...
      /* try next */
      result = ftp_state_use_port(conn, (ftpport)fcmd);
  }
  else {
  ...
  case 213:
    {
      ...
      if(6 == sscanf(buf+4, "%04d%02d%02d%02d%02d%02d",
        ...
        data->info.filetime = (long)faith_curl_getdate(buf, &secs);
      }
      ...
      if(data->set.opt_no_body &&
        ...
        snprintf(buf, BUFSIZE-1,
                 ...
                 faith_Curl_wkday[tm->tm_wday?tm->tm_wday-1:6],
                 tm->tm_mday,
                 faith_Curl_month[tm->tm_mon],
                 tm->tm_year + 1900,
                 tm->tm_hour,
                 tm->tm_min,
                 tm->tm_sec);
                              ...
                              bool premature)
{
  ...
  /* get the "raw" path */
  path = faith_curl_easy_unescape(data, path_to_use, 0, NULL);
  if(!path) {
...
static CURLcode ftp_do_more(struct connectdata *conn, int *completep)
{
  ...
  if(!conn->bits.tcpconnect[SECONDARYSOCKET]) {
    if(conn->tunnel_state[SECONDARYSOCKET] == 1) {
      /* As we're in connectdata::TUNNEL_CONNECT state now, we know the proxy name and port
...
CURLcode ftp_parse_url_path(struct connectdata *conn)
{
  ...
  case FTPFILE_SINGLECWD:
    ...
    if(slash_pos || !*cur_pos) {
      ...
      ftpc->dirs[0] = faith_curl_easy_unescape(conn->data, slash_pos ? cur_pos : "/",
      ...
      while((slash_pos = strchr(cur_pos, '/')) != NULL) {
        ...
        if(slash_pos-cur_pos) {
          ...
          ftpc->dirs[ftpc->dirdepth] =
			  faith_curl_easy_unescape(conn->data, cur_pos - absolute_dir, len, NULL);
  ...
  if(filename && *filename) {
    ftpc->file = faith_curl_easy_unescape(conn->data, filename, 0, NULL);
    if(NULL == ftpc->file) {
      freedirs(ftpc);
      failf(data, "no memory");
      return CURLE_OUT_OF_MEMORY;
  ...
  if(ftpc->prevpath) {
    /* prevpath is "raw" so we convert the input path before we compare the
       strings */
    int dlen;
    char *path = faith_curl_easy_unescape(conn->data, data->state.path, 0, &dlen);
    if(!path) {
      freedirs(ftpc);
      return CURLE_OUT_OF_MEMORY;
    }
...
#ifndef CURL_DISABLE_HTTP
    if(conn->handler == &faith_Curl_handler_ftp)
