...
namespace faith 
{
	namespace net
	{
		...
		unsigned int tcp_client::connect_to(unsigned int conn_index, xstring ip,xstring service_port,
			connection_handler_type connection_handler,
			onclose_handler_type onclose_handler,
			onrecv_handler_type onrecv_handler)
		{
			VMPBEGIN
			unsigned int ret = impl_ptr->connect_to(conn_index, ip,service_port,connection_handler,onclose_handler,onrecv_handler);
			VMPEND
			return ret;
		}

		void tcp_client::disconnect(unsigned int conn_index)
		{
			impl_ptr->disconnect(conn_index);
		}

		int tcp_client::send(unsigned int conn_index,const void *data_ptr,size_t data_len)
		{
			VMPBEGIN
			int ret = impl_ptr->send(conn_index,data_ptr,data_len);
			VMPEND
			return ret;
		}

		int	tcp_client::send_multi(unsigned int conn_index,const datablock_queue_type& data_queue)
		{
		...
		void tcp_client::init_client_server(unsigned int server_num)
		{
